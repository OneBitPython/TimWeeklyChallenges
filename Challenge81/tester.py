...
def print_begin(
    ...
    if PRINT_EXTRA_STATS:
        print_extra_stats(test_inp[:num_cases],
                          test_out[:num_cases], num_cases)
    if (
        solution_len
        ...
        print(
            f" - Solution length{emojis.scroll}: {green}{solution_len}{reset} chars.")
    print()
...
def print_summary(i: int, passed: int, end: float, start: float) -> None:
    ...
    print(
        f"Passed: {green if passed == i else red}{passed}/{i}{reset} tests{e}{emojis.bang}")
...
def test_solution_aio(
    ...
    test_inp_: List[str] = functools.reduce(
        operator.iconcat, test_inp[:num_cases], [])
...
def speed_test_solution_aio(
    ...
    print(
        f'Testing {cyan}{speed_num_cases}{reset} cases, format "All in one":')
...
def test_solution_obo(
    ...
    print(
        f"\rPassed: {green if passed == i + 1 else red}{passed}/{i + 1}{reset} tests")
...
def speed_test_solution_obo(
    ...
    print(f"Running: {yellow}Python solution{reset}")
    print(
        f'Testing {cyan}{speed_num_cases}{reset} cases, format "One by one":')
    print(f" - Number of loops: {cyan}{loops:_}{reset}")
    print(f" - Timeout: {red}{TIMEOUT}{reset} seconds")
    if PRINT_EXTRA_STATS:
        print_extra_stats(
...
def test_other_lang(
    ...
    test_inp_ = (
        f"{num_cases}\n"
        + "\n".join(functools.reduce(operator.iconcat,
                    test_inp[:num_cases], []))
        + "\n"
    )
    print_begin("All in one", num_cases, test_inp, test_out)
...
def main(path: str) -> None:
    ...
    if DEBUG_TEST:
        case_number = (
            DEBUG_TEST_NUMBER if 0 <= DEBUG_TEST_NUMBER -
            1 < len(test_out) else 1
        )
